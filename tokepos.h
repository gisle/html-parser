...
#define PUSH_TOKEN(p_beg, p_end) \
   STMT_START {
     ...
     tokens[num_tokens].beg = p_beg; \
     tokens[num_tokens].end = p_end; \
   } STMT_END
...
{
  int new_lim = *token_lim_ptr * 2;
  if (realloc) {
    Renew(*token_ptr, new_lim, struct token_pos);
  }
  else {
    struct token_pos *new_tokens;
    int i;
    New(57, new_tokens, new_lim, struct token_pos);
